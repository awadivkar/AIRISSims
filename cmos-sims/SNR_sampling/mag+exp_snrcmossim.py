...
def calculate_image_flux(image_magnitude, sensor_params, exposure_time):
    ...
    total_photons = total_flux * exposure_time  # Adjust based on exposure time
...
def generate_image(params, exposure_time, magnitude, binning=False, cosmic_rays=False, sky_background=False, moving_exposures=False, snr_calc=False, pbar=None, win=None
    ...
    x_positions, y_positions, magnitudes = [int(params["Sensor Height (px)"])/2], [int(params["Sensor Width (px)"])/2], [magnitude]
    ...
    if moving_exposures:
        ...
        for step in range(num_steps):
            sub_exposure_time = exposure_time / num_steps
    ...
    dark_noise = np.random.poisson(params["Dark Current (e-)"] * exposure_time, image.shape)
    ...
    if binning:
        if debug: print('Applying 3x3 binning')
    ...
    if snr_calc:
        ...
        with open('cmos-sims/SNR_sampling/mag+expstats.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            # Write the header if the file is empty
            if csvfile.tell() == 0:
                writer.writerow(["Mask Radius", "Exposure Time", "Magnitude", "Signal", "Noise", "SNR"])
            # Write the SNR stats
            writer.writerow([r_ap, exposure_time, magnitude, excess_flux, noise_emp, SNR_empirical])
...
def run_simulation():
    ...
    uniform_snr = snr_uniform_var.get()

    for i in tqdm(range(int(params["Candidates: "] - 1)), desc="Simulating Candidates", leave=True):
        for j in range(int(params["Samples per Candidate"])):
            res = int(np.sqrt(int(params["Candidates: "])))
            if uniform_snr:
                run_exp_time = params["Min Exposure Time"] + ((i % res) * (params["Max Exposure Time"] - params["Min Exposure Time"]) / (res - 1))
                run_mag = params["Min Magnitude"] + (np.floor(i/res) * (params["Max Magnitude"] - params["Min Magnitude"]) / (res - 1))
            else: 
                run_exp_time = np.random.uniform(params["Min Exposure Time"], params["Max Exposure Time"])
                run_mag = np.random.uniform(params["Min Magnitude"], params["Max Magnitude"])

            # run_mag = params["Max Magnitude"] - (i * (params["Max Magnitude"] - params["Min Magnitude"]) / (params["Candidates: "] - 1))
            # print(run_mag)
            image = generate_image(params, run_exp_time,
                                    run_mag,
...
tk.Label(title_frame, text="CMOS Image Simulation (SNR)", font=("TkDefaultFont", 24, 'bold')).grid(row=0, column=0, sticky="w")
...
imgshow_var = tk.BooleanVar(value=False)
moving_exposures_var = tk.BooleanVar(value=False)
snr_calc_var = tk.BooleanVar(value=True)
snr_uniform_var = tk.BooleanVar(value=False)
...
tk.Checkbutton(toggle_frame, text="Uniform SNR Sampling", variable=snr_uniform_var).grid(row=3, column=0, sticky='w')
