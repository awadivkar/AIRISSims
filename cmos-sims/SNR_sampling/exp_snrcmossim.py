...
DEFAULTS = {
    ...
    "Magnitude": 0,
    "Candidates: ": 100,
    "Samples per Candidate": 100,
    "Zero Point": 18,
    "PSF (sigma)": 3,
    "Min Exposure Time": 1,
    "Max Exposure Time": 250,
}
...
def generate_image(params, exposure_time, binning=False, cosmic_rays=False, sky_background=False, moving_exposures=False, snr_calc=False, pbar=None, win=None
    ...
    x_positions, y_positions, magnitudes = [int(params["Sensor Height (px)"])/2], [int(params["Sensor Width (px)"])/2], [params["Magnitude"]]
    ...
    if snr_calc:
        ...
        with open('expstats.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            # Write the header if the file is empty
            if csvfile.tell() == 0:
                writer.writerow(["Mask Radius", "Exposure Time", "Signal", "Noise", "SNR"])
            # Write the SNR stats
            writer.writerow([r_ap, exposure_time, excess_flux, noise_emp, SNR_empirical])
