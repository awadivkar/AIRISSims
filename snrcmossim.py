...
def generate_image(params, magnitude, binning=False, cosmic_rays=False, sky_background=False, moving_exposures=False, snr_calc=False, pbar=None, win=None
    ...
    if snr_calc:
        ...
        if debug: print("Mask Radius: ", radius, ", Signal: ", signal, ", Noise: ", noise, ", SNR: ", signal / noise)

        with open('snrstats.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            # Write the header if the file is empty
            if csvfile.tell() == 0:
                writer.writerow(["Mask Radius", "Magnitude", "Signal", "Noise", "SNR"])
            # Write the SNR stats
            writer.writerow([radius, magnitude, signal, noise, signal / noise])
...
def run_simulation():
    ...
    for i in tqdm(range(int(params["Candidates: "] - 1)), desc="Simulating Candidates", leave=True):
        for j in range(int(params["Samples per Candidate"])):
            ...
            # print(run_mag)
            image = generate_image(params, run_mag,
...
logo_img = Image.open("NewDLogo.png")
